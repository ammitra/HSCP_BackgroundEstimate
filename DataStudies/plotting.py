def plotRPF(postfitShapesFile,odir):
    ...
    f, ax = plt.subplots()
    hep.histplot(hRpf,edges[0],label = "$R_{P/F}$", histtype="step")
...
def plotPostfit(postfitShapesFile,region,odir):
    ...
    plotShapes(projections[0],projections[1:],uncBand_proj,labels[1:],colors[1:],"$I_{as}$","{0}/postfit_{1}.png".format(odir,region),logFlag=True)
